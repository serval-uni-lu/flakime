...
public class FlakimeInstrumenter {
     ...
     */
    public static void instrument(TestMethod testMethod, Strategy strategy)
        ...
        testMethod.insertBefore(
                String.format("%s = %f;", randomVariableName, Math.random()));
        testMethod.insertBefore(String.format(
                "%s = Boolean.parseBoolean(System.getenv(\"FLAKE_FLAG_FLAKIME\"));",
                flakimeInjectorFlag));

        for (int lineNumber : testMethod.getStatementLineNumbers()) {
            final String payload =
                    computePayload(testMethod, strategy, lineNumber);
     ...
     */
    private static String computePayload(TestMethod testMethod,
                                         Strategy strategy, int lineNumber) {
